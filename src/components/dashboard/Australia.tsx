...
import {fetchAustraliaData} from '../../api/dashboard/index';
interface IProps{
}
interface IState{
...
class Australia extends Component <{}, IState> {
    ...
    componentDidMount(){
      const fetchMyAPI = async () => {
          const initialDailyData:any = await fetchAustraliaData();
          this.setState({
            data: initialDailyData.regions.australia.list,
            dataTotal: initialDailyData.regions.australia.totals,
  ...
  render(){
    ...
    sortedInfo = sortedInfo || {};
      ...
      var dataAustralia:any = this.state.dataTotal;
        const data2 = [];
        data2.push(
          {key: 0,
          country: <p className="label label-danger">TOTAL</p>,
          confirmed: dataAustralia.confirmed,
          newConfirmed: dataAustralia.daily_deaths,
          death:dataAustralia.deaths,
          newDeath: dataAustralia.daily_deaths ,
          recovered:dataAustralia.recovered,
        ...
        for (var i=0; i<newColumns.length;i++)
        {
            data2.push({
                ...
                key: i,
                country: <div>
                  <img src="https://cdn.jsdelivr.net/gh/hjnilsson/country-flags@latest/svg/au.svg" width="40" alt="Flag China"/> {newColumns[i].state}
                </div>,
