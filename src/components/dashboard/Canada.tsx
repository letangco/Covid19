...
import {fetchCanadaData} from '../../api/dashboard/index';
interface IProps{
}
interface IState{
    data :any [],
...
class Canada extends Component <{}, IState> {
    ...
    componentDidMount(){
      const fetchMyAPI = async () => {
          const initialDailyData:any = await fetchCanadaData();
          this.setState({
            data: initialDailyData.regions.canada.list,
            dataTotal: initialDailyData.regions.canada.totals,
  ...
  render(){
    ...
    sortedInfo = sortedInfo || {};
      ...
      var dataCanada:any = this.state.dataTotal;
        const data2 = [];
        data2.push(
            {key: 0,
            country: <p className="label label-danger">TOTAL</p>,
            confirmed: dataCanada.confirmed,
            newConfirmed: dataCanada.daily_deaths,
            death:dataCanada.deaths,
            newDeath: dataCanada.daily_deaths ,
            recovered:dataCanada.recovered,
        ...
        for (var i=0; i<newColumns.length;i++)
        {
            data2.push({
                ...
                key: i,
                country: <div>
                  <img src="https://cdn.jsdelivr.net/gh/hjnilsson/country-flags@latest/svg/ca.svg" width="40" alt="Flag USA"/>
                  <span> {newColumns[i].state}</span>
                </div>,
