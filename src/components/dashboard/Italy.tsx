...
import {fetchItalyData} from '../../api/dashboard/index';
interface IProps{
}
interface IState{
...
class Italy extends Component <{}, IState> {
    ...
    componentDidMount(){
      const fetchMyAPI = async () => {
          const initialDailyData:any = await fetchItalyData();
          this.setState({
            data: initialDailyData.regions.italy.list,
            dataTotal: initialDailyData.regions.italy.totals,
  ...
  render(){
    ...
    sortedInfo = sortedInfo || {};
      ...
      var dataItaly:any = this.state.dataTotal;
        const data2 = [];
        data2.push(
          {key: 0,
          country: <p className="label label-danger">TOTAL</p>,
          confirmed: dataItaly.confirmed,
          newConfirmed: dataItaly.daily_deaths,
          death:dataItaly.deaths,
          newDeath: dataItaly.daily_deaths ,
          recovered:dataItaly.recovered,
        ...
        for (var i=0; i<newColumns.length;i++)
        {
            data2.push({
                ...
                key: i,
                country: <div>
                  <img src="https://cdn.jsdelivr.net/gh/hjnilsson/country-flags@latest/svg/it.svg" width="40" alt="Flag China"/> {newColumns[i].state}
                </div>,
